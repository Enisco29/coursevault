    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Dialog open={openUpload} onOpenChange={setOpenUpload}>
            ...
            <DialogContent className="max-w-2xl">
              <DialogHeader>
                ...
                <DialogDescription>Upload one or more PDF files to this folder</DialogDescription>
              </DialogHeader>
              <UploadZone
                courseId={courseId}
          ...
          <CreateFolderDialog courseId={courseId} parentFolderId={parentFolderId} />
        </div>
      </div>
      ...
      {folders.length === 0 && documents.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 text-center border border-dashed rounded-lg">
          ...
          <p className="text-muted-foreground">Create a folder or upload documents to get started</p>
        </div>
      ) : (
        <>
          {folders.length > 0 && (
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {folders.map((folder) => (
                  <button
                    key={folder.id}
                    onClick={() => router.push(`/dashboard/course/${courseId}/folder/${folder.id}`)}
